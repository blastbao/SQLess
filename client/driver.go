...
import (
	...
	bp "github.com/SQLess/SQLess/blockproducer"
	"github.com/SQLess/SQLess/blockproducer/interfaces"
...
func init() {
	...
	log.Debug("SQLess driver registered.")
}
...
func defaultInit() (err error) {
	...
	if configFile == DefaultConfigFile {
		...
		log.Debugf("Could not find SQLess default config location: %v", configFile)
		return ErrNotInitialized
	}

	log.Debugf("Using SQLess default config location: %v", configFile)
...
func WaitDBCreation(ctx context.Context, dsn string) (err error) {
	...
	db, err := sql.Open("cqlprotocol", dsn)
	defer db.Close()
	if err != nil {
		return
	}
